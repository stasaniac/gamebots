...
class RandHandStrengthBot(Bot):
    ...
    def get_move(self, state: 'PlayerPerspective', leader_move: Optional['Move']) -> 'Move':
        ...
        move = RandHandStrengthBot.hand_value_bot(state, leader_move, valid_moves, trumpSuit)

        return move

        
    def hand_value_bot(state, leader_move, valid_moves, trumpSuit) -> 'Move':
        randint = random.randint(0,1)


        if randint == 1:
            move = RandHandStrengthBot.agressive_move(state, leader_move, valid_moves, trumpSuit)

        else: 
            move = RandHandStrengthBot.passive_move(state, leader_move, valid_moves, trumpSuit) 
